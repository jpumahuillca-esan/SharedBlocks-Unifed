...
import ModalTeamUniversity from '../../_shared/ModalTeamUniversity.vue'
...
interface BlockData {
  ...
  limit?: number | string
  forcedType?: string
  // ── 🎯 NUEVAS PROPS DE NAVEGACIÓN DINÁMICA DEL PAGE BUILDER
  actionText?: string
  actionUrl?: string
}
...
// ── 🎯 PARAMETRIZACIÓN DE BOTÓN COMPARTIDO
...
// ── 🚩 ESTADOS Y CONTROLADORES REACTIVOS DE TU MODAL INTACTO ──
...
const fetchDirectorioMiembros = async () => {
  try {
    // Disparamos la acción mediante el hilo unificado del composable
...
const injectMockUniversidades = () => {
  ...
  if (!isNuxtEnvironment) {
    items.value = [
      { id: 1, name: 'San Cristóbal de Huamanga', siglas: 'UNSCH', logo_url: 'https://upload.wikimedia.org/wikipedia/commons/4/46/Escudo_de_la_Universidad_Nacional_Mayor_de_San_Marcos.svg', type: 'public' },
      { id: 2, name: 'De Ucayali', siglas: 'UNU', logo_url: 'https://upload.wikimedia.org/wikipedia/commons/4/46/Escudo_de_la_Universidad_Nacional_Mayor_de_San_Marcos.svg', type: 'public' },
      { id: 3, name: 'Tecnológica de Lima Sur', siglas: 'UNTELS', logo_url: 'https://upload.wikimedia.org/wikipedia/commons/4/46/Escudo_de_la_Universidad_Nacional_Mayor_de_San_Marcos.svg', type: 'public' },
      { id: 4, name: 'De Tumbes', siglas: 'UNT', logo_url: 'https://upload.wikimedia.org/wikipedia/commons/4/46/Escudo_de_la_Universidad_Nacional_Mayor_de_San_Marcos.svg', type: 'public' },
    ...
    <div class="member-directory__outer">
      <div class="member-directory__inner">
        ...
        <div v-if="pending" class="member-directory__loader">
          ...
          <AtomText class="member-directory__loader-text">Sincronizando universidades miembro...</AtomText>
        </div>
        ...
        <div v-else-if="universidadesFiltradas.length > 0" class="member-directory__grid">
          ...
          >
            ...
            <div class="university-card__body">
              ...
              <AtomHeading :level="3" class="university-card__name">
                {{ uni.name }}
              </AtomHeading>
            </div>
          </article>
        ...
        <div v-else class="member-directory__empty">
          ...
          <AtomText class="member-directory__empty-text">
...
.member-directory__outer { width: min(100%, 1010px); margin: 0 auto; padding: 28px; box-sizing: border-box; background: #ffffff; border-radius: 8px; }
