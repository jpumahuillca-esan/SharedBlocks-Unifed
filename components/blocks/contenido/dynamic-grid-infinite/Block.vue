  ...
  <section class="feed-infinite-section">
    <div class="feed-infinite-shell">
      <div class="section-header-wrapper mb-10">
        ...
        <AtomText v-if="introText" class="feed-section-intro">
          {{ introText }}
        </AtomText>
      </div>
        ...
        >
          <a :href="getDetailLink(item)" class="feed-card-link">
            ...
            <div class="container__content">
              ...
              <AtomHeading :level="3" class="card-core-headline-title" :title="item?.title || 'Sin título'">
                <span class="headline-text-span">
                  {{ item?.title || 'Sin título' }}
                </span>
              </AtomHeading>

              <AtomText class="card-excerpt-description" v-if="item?.excerpt">
                {{ item.excerpt }}
              </AtomText>
            </div>
      ...
      <div v-else class="feed-empty-fallback-box">
        <AtomText>No se encontraron publicaciones vigentes bajo el filtro de este bloque.</AtomText>
      </div>
