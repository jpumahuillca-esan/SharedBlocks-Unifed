...
const fetchDirectorioAutoridades = async () => {
  pending.value = true
  try {
    const url = `${PUBLIC_API_URL}/teams?hierarchy_level=1&person_type=board_member&limit=100`
...
/**
 * Adaptador ad-hoc para cumplir estrictamente la firma original de tu Modal
 */
...
<template>
  <section class="directorio-section-wrapper">
    <div class="directorio-fluid-container">
      ...
      <div class="filters-row-grid text">
        ...
        <!--<div v-for="label in ['Vicerrector Académico', 'Vicerrector de Investigación']" :key="label" class="disabled-placeholder-card">
          <span class="placeholder-label-text">{{ label }}</span>
          <svg class="chevron-arrow-svg text-slate-disabled" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
          </svg>
        </div>-->
      </div>

      <!--<div class="filters-row-grid mb-gutter">
        <div v-for="label in ['Director General de Administración', 'Secretaría Técnico', 'Consejo Directivo', 'Comité Técnico Interáreas']" :key="label" class="disabled-placeholder-card">
          <span class="placeholder-label-text text-left-d">{{ label }}</span>
          <svg class="chevron-arrow-svg text-slate-disabled" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
          </svg>
        </div>
      ...
      <div v-else-if="autoridadesVisibles.length > 0" class="authorities-cards-grid">
        ...
        >
          ...
          <div class="profile-info-side force-no-pointer">
            ...
            <AtomHeading :level="3" class="authority-fullname">
      ...
      <div v-else class="empty-fallback-box">
        ...
        <AtomText>No se encontraron autoridades registradas bajo este filtro</AtomText>
      </div>
