    ...
    <div class="colaboramos-section__outer">
      <div class="colaboramos-section__inner">
        <header class="colaboramos-section__header">
          <AtomHeading :level="2" class="colaboramos-section__title" v-if="title">
            {{ title }}
          </AtomHeading>

          <AtomText class="colaboramos-section__intro" v-if="introText">
            {{ introText }}
          </AtomText>
        </header>
        ...
        <div class="colaboramos-section__cards" v-if="cards.length > 0">
          <article v-for="(item, index) in cards" :key="item.id || index" class="colaboramos-card">
            ...
            <AtomHeading :level="3" class="colaboramos-card__title" v-if="item.title">
              {{ item.title }}
            </AtomHeading>

            <AtomText class="colaboramos-card__text" v-if="item.text">
              {{ item.text }}
            </AtomText>
          </article>
        </div>
        ...
        <article class="colaboramos-card colaboramos-card--wide" v-if="wideCard">
          ...
          <div class="colaboramos-card--wide__body">
            <AtomHeading :level="3" class="colaboramos-card__title" v-if="wideCard.title">
              {{ wideCard.title }}
            </AtomHeading>

            <AtomText class="colaboramos-card__text" v-if="wideCard.text">
              {{ wideCard.text }}
            </AtomText>
          </div>
        </article>
